...
public class NetworkManager : MonoBehaviour
{
    ...
    [HideInInspector] public int port = 4296;
	...
	void ReceiveData(ushort senderID, byte tag, ushort subject, object data)
    {
		...
		if(tag == TagIndex.Controller)
		{
			...
			if(subject == TagIndex.ControllerSubjects.JoinMessage)	
			{
				...
				DarkRiftAPI.SendMessageToID (senderID, TagIndex.Controller, TagIndex.ControllerSubjects.SpawnPlayer, localPlayer.position);
			}
			...
			if(subject == TagIndex.ControllerSubjects.SpawnPlayer)
			{
				...
				GameObject clone = (GameObject)Instantiate(playerPrefab, (Vector3)data, Quaternion.identity);
