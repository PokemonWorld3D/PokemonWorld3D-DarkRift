...
//	public float targetHeight = 1.7f;					//Vertical offset adjustment
    ...
    public void CameraAim(float xMin, float xMax, float yMin, float yMax)
    {
        curXMin = target.rotation.eulerAngles.y + xMin;
        curXMax = target.rotation.eulerAngles.y + xMax;
        curYMin = target.rotation.eulerAngles.x + yMin;
        curYMax = target.rotation.eulerAngles.x + yMax;
    }
